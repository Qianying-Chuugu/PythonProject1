...
> 拿它跟现在的绝对 0.45 在 25 条查询（21 条有答案 + 4 条语料外探针）上逐条对比
> （`tools/benchmark.py` 第四节），结果是**它在本项目唯一能测的场景里严格更差**：
>
> | 分数线 | 答案排第1 | 有答案平均返回 | 语料外平均返回 | 放行率 |
> | --- | --- | --- | --- | --- |
> | **绝对 0.45（现用）** | 21/21 | 10.9 个文件 | **1.8 个文件** | 19.5% |
> | 均值 + 0.0×σ（只留高于平均） | 21/21 | 18.2 | 19.2 | 47.5% |
> | 均值 + 0.5×σ | 21/21 | 12.7 | 14.2 | 27.5% |
> | 均值 + 1.25×σ | 21/21 | 5.0 | 7.8 | 8.9% |
> | 分数最高前 5%（对照） | 21/21 | 3.8 | 5.0 | 5.7% |
>
> 「排第 1」一列**全是打平**，没有一条规则更高；而**语料外返回文件数全都更差**
> （现状 1.8 个，相对线最好的也要 5.0 个）。摆平"有答案返回数"再比更清楚：
> 现状 10.9 个文件时语料外只漏 1.8 个，相对线要做到 10 个文件（前 20% = 10.3）
...
> 地板加狠一点，就等于又变回绝对阈值了。
>
> 一个诚实的保留：**绝对线 0.45 是在这 141 个段落上校准出来的，有主场优势。**

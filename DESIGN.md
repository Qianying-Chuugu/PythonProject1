...
> 只有第一个是尖峰，另两个离 0.45 那条线远得很，不是"刚过线"。结论（平均救了它）
> 是对的，机制描述是错的——写文档时是**推断**，没量过，现在量了才发现。

#### 二维扫描：为什么"改成前 5 段平均"是个陷阱

`min_score` 同时是**过滤器**和**取前 N 段的取材范围**（NOTES 第 11 条），所以固定
0.45 只换 `_TOP_N` 量出来的结论不算数——得两维一起扫（`tools/benchmark.py` 第六节，
纯本地计算，不过模型）。
...
| 门槛 | 0.30 | 0.40 | **0.45（现状）** | 0.50 | 0.55 | 0.60 | 0.65 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 有答案的查询 | 21.5 | 16.6 | **10.9** | 5.0 | 2.3 | **1.6** | 1.1 |
| 语料外的查询 | 14.2 | 7.0 | **1.8** | 0.2 | 0.0 | 0.0 | 0.0 |

`0.60` 那格"最小领先 0.040、6.1 倍"是**假的**：它靠的是把「有答案」的查询平均压到
**1.6 个文件**（现在 10.9 个）。**把结果删光，第 1 名当然领先得舒服。**
「最小领先」这个新指标跟旧的「答案排第几」一样，对"列表缩水"是瞎的——换了个指标，
同一个盲点又犯一次（NOTES 第 13 条）。**所以任何门槛/合成规则的主张都要配表 C 看。**

**副产品（记在 ROADMAP，本轮不动）**：门槛 `0.50` 看起来是白捡的——排第 1 仍是 21/21、
最小领先 0.007 → 0.015、**语料外返回文件数 1.8 → 0.2**（上一轮改相对阈值想要却没拿到
的效果），而「有答案」是 5.0 个文件——`search_semantic` 默认 `top_k = 5`，**用户本来
就只看得到 5 条**，所以可见结果一条没少。

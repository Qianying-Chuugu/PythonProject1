...
def _测量(args, 语料):
    ...
    print("=" * 72)
    print(f"门槛固定 {当前默认阈值}（跟现状一致），只换合成规则。**不过模型、不改生产代码。**")
    ...
    # （NOTES.md 第 11 条），所以把 _TOP_N 从 3 改成 5，0.45 这个数的含义就跟着变了。
    # 固定 0.45 只换合成规则量出来的结论，不足以支撑"那就改成 5"。
    #
    # 好消息：这一节**仍然一次模型都不用调**——段落分数已经在内存里。
    print("\n" + "=" * 72)
    print("六、二维扫描：min_score × 合成规则")
    print("=" * 72)
    print("上一节是**固定 0.45** 只换合成规则量的。但这两个参数是耦合的：")
    print("min_score 同时是「过滤器」和「取前 N 段的取材范围」（NOTES.md 第 11 条）。")
    print("把 N 从 3 改成 5，过线的段落就变多，0.45 的含义跟着变——")
    print("所以得两维一起扫才敢下结论。**这一节也不调模型。**\n")

    二维阈值 = sorted({0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 当前默认阈值})
    二维规则 = [
        ("只取最高分", lambda h: 前k平均(h, 1)),
        ("前 2 段平均", lambda h: 前k平均(h, 2)),
    ...
    print("      「有答案」的查询平均只剩 1.6 个文件（现在 10.9 个）。**把结果删光，")

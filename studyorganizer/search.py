...
def search_semantic(query, top_k=5, min_score=0.45):
    ...
    但比原来用的 text2vec 好得多：同样卡 0.45，text2vec 放行 49% 的段落（阈值形同
    虚设），bge-small 只放行 27%；正确答案的文件分大约在 0.58~0.76。
    换模型时**刻意没有重调这个数**，也没打算靠调它来提升效果：实测 0.45~0.60 这
    一段 6 条测试查询全对，再往上（0.65）反而掉到 5/6——调高不会更准，只会把有用的
    段落一起砍掉。0.45 的定位始终只是"砍掉明显跑题的尾部"；真正决定顺序的是排序，

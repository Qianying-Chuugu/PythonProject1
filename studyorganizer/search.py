...
# 算文件分时，最多拿最好的几段来平均。
# 这个 3 **量过**（tools/benchmark.py 第五、六节），结论是**不改**——但不是因为"它最好"，
# 而是因为它**不重要**：二维扫描显示，在现在的默认门槛 0.50 上，前 2 / 前 3 / 前 5 段
# 平均给出的最小领先**一模一样**（都是 0.015）。真正的杠杆是 min_score，不是这个数。
# 见 DESIGN.md「这个合成规则到底在干什么」。
_TOP_N = 3
...
def search_semantic(query, top_k=5, min_score=0.50):
    """
    语义搜索：返回和 query 意思最相近的 top_k 个文件。
    参数 min_score：段落相似度下限，低于它的段落直接丢掉（0~1，越大越严格）。
    返回 [(标题, 分数, 原因), ...]，分数从高到低。

    关于默认门槛 0.50：这是**实测**出来的，不是拍的，而且随时能自己重跑验证
    ——`python tools/benchmark.py`（数字和对照表在 DESIGN.md）。
    bge-small 给出的相似度会"压缩"（跟查询无关的段落大多落在 0.2~0.55），
    所以门槛得设在中段才砍得动跑题的尾部。

    它跟 0.45 的关系（0.45 是 2026-09 之前的值，都是从 21 条带标准答案的查询 +
    4 条**语料外**探针上量出来的）：
      · 「答案排第 1」两条一样，都是 21/21；
      · 「最小领先」（答案分 − 最高的非答案文件分）0.007 → 0.015，最薄那条厚了一倍；
      · 「语料外查询返回几个文件」1.75 → **0.25**（问语料里没有的东西时基本返回空，
        4 条探针里 3 条一条都不返回）；
      · 「有答案的查询返回几个文件」10.9 → 5.0——看着少了，但默认 top_k 就是 5，
        **用户本来只看得到 5 条**，所以可见结果一条没少。
    所以这不是"更严"，是**同样准、但更不容易被错文件顶掉、而且不硬凑**。

    **别再往上加了。** 0.55 就会把"有答案"的查询压到平均 2.3 个文件、0.60 压到 1.6，
    用户看到的结果开始真的变少（DESIGN.md 表 C）。而且它不是一个"越高越严"的旋钮：
    0.45~0.60 都是 21/21，0.65 崩到 17/21（答案段落被整条砍掉），
    **0.30/0.40 反而掉到 20/21**。原因是 min_score 不只是过滤器，它还决定
    "前 3 段平均"从哪些段里取，放宽会把文件分拖下来（NOTES.md 第 11 条）。
    真正决定顺序的是排序，不是这个阈值。
